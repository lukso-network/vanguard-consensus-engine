...
type EventSystem struct {
	...
	consensusInfoCh chan *MinimalEpochConsensusInfo // Channel to receive new new consensus info event
}
...
func NewEventSystem(backend Backend) *EventSystem {
	m := &EventSystem{
		...
		consensusInfoCh: make(chan *MinimalEpochConsensusInfo, 1),
	}
...
func (es *EventSystem) SubscribeConsensusInfo(consensusInfo chan *MinimalEpochConsensusInfo, epoch eth2Types.Epoch) *Subscription {
	sub := &subscription{
		id:            rpc.NewID(),
...
func (es *EventSystem) handleConsensusInfoEvent(filters filterIndex, ev *MinimalEpochConsensusInfo) {
...
// sendConsensusInfo
func (es *EventSystem) sendConsensusInfo(f *subscription, ev *MinimalEpochConsensusInfo) {
	ctx := context.Background()

	curEpoch := eth2Types.Epoch(0)

	log.WithField("f.epoch", f.epoch).Debug("subscriber's epoch status")
	ev, err := es.backend.GetMinimalConsensusInfo(ctx, curEpoch)
	if nil != err {
		return
	}
