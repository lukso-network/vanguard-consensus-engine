...
func (s *Service) onBlock(ctx context.Context, signed interfaces.SignedBeaconBlock, blockRoot [32]byte) error {
	...
	if s.enableVanguardNode {
		...
		signedBeaconBlock, err := signed.PbPhase0Block()

		if nil != err {
			return errors.Wrap(err, "could not verify pandora sharding info")
		}

		if err := s.verifyPandoraShardInfo(signedBeaconBlock); err != nil {
